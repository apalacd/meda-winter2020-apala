...
//declare a variable history object to hold all submission history for /sayHello
...
if (fs.existsSync("history.json")){
    ...
    //convert a json to a string.  If it does exist, convert history into JSON and save it to history.json
    
    let json = JSON.stringify(history);
    //pass to the writeFileSync what, where it's going to save, in what format
    // writeFileSync("json file", {json Object}, string format)
...
app.post("/sayHello", (request, response) => {
    ...
    let userNummberChoice = parseInt(dataFromFront.number);

    let historyEntry = {
        number: userNumberChoice,
        winningNumber: winningNumber,
        // timestamp when the history object got posted
        timestamp: Date.now()
    }
    ...
    if (userNumberChoice <= 0 || userNumberChoice >= 11) {
        // If it is, set the outOfRange variable to true.
        outOfRange = true;
    }

    //Build an objecct to send back to whoever requested this specific POST method.
//    let responseObject = {};
    ...
    //req and res are commonly used for request and response 
app.post("/getPreviousEntries", (req, res) => {
    let dataToSendBack = {
        latestEntries: history.submissions    // take the entire array stored as part of the history object and copy it to the latesEntries
    }
